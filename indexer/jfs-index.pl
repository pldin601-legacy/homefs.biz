...
my $ff_cmd = "/usr/local/bin/ffmpeg";
...
sub load_tof {
	%tof = ();
	$q = $dbh->prepare("SELECT * FROM `extensions` WHERE 1");
	$q->execute();
	while(@arr = $q->fetchrow_array()) {
		$tof{$arr[0]} = $arr[1];
	}
}
...
sub scan_file {
	...
	$q = $dbh->prepare("SELECT `id`,`modified`,`md5` FROM `files` WHERE `name` = ? AND `dir_id` = ?");
	...
	if($q->rows == 1) {
		...
		$fid = $dbh->last_insert_id(undef, undef, "files", "id");
		mark_reindexed($parent_id);
		if(has_defined_elements(@meta)) {
